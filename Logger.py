...
testpayload = bytearray( b'\x02\x01"\x03\x8d\xfcn\x00 \x03\x90\xfco\x00\x1e\x03\x8d\xfcp\x00"\x03\x90\xfcn\x00"\x03\x8e\xfcq\x00!\x03\x8e\xfcl\x00!\x03\x8d\xfcn\x00 \x03\x8e\xfcq')

def WriteMetaToFile(dev_id, payload, Metadata, date):
    ...
    if header == 4:
        ...
        payload_data = payload[1:]
        dev_timestamp = int.from_bytes(payload[1:5], byteorder='big', signed=True)
    ...
    try:
        ...
        templist = []
        if header == 2:
            templist.append(servertime)
            templist.append(dev_timestamp)
        try:
...
WriteMetaToFile("lorakeypad", testpayload, meta, some_date)
WriteMetaToFile("lorakeypad", testpayload, meta, some_date)
